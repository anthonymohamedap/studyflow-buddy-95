...
serve(async (req) => {
  ...
  try {
    ...
    let fileContent: string | undefined = documentContent;
    ...
    const middlewareRes = await fetch(`${middlewareUrl}/parse-document`, {
      ...
      body: JSON.stringify({
        ...
        filename: filePath ? filePath.split("/").pop() : "document.pdf",
    ...
    for (let ci = 0; ci < chapters.length; ci++) {
      ...
      if (chapterError) { console.error("Chapter insert fout:", chapterError); continue; }
